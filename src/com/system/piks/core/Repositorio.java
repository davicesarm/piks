...
public class Repositorio {
    ...
    private static final Map<String, Conta> contasPIKS = new TreeMap<>();
    private static final Map<Integer, Cliente> clientesCPF = new TreeMap<>();
	...
	public static void adicionarConta(Conta c) {
        if (contasPIKS.containsKey(c.getChavePiks())) 
            throw new IllegalArgumentException("Conta com chave " + c.getChavePiks() + " ja existe");
        if (c.getCliente() == null)
            throw new IllegalArgumentException("Conta deve ter um cliente");
        if (clientesCPF.containsKey(c.getCliente().getCpf()))
            throw new IllegalArgumentException("Cliente com CPF " + c.getCliente().getCpf() + " ja possui uma conta");
	...
	public static void removerConta(Conta c) {
        if (!contasPIKS.containsKey(c.getChavePiks())) 
            throw new IllegalArgumentException("Conta com chave " + c.getChavePiks() + " nao existe");
        if (c.getSaldo() != 0.0)
            throw new IllegalArgumentException("Conta com chave " + c.getChavePiks() + " possui saldo diferente de zero");
        if (c.getCliente() != null && clientesCPF.containsKey(c.getCliente().getCpf()))
            throw new IllegalArgumentException("Conta com chave " + c.getChavePiks() + " possui um cliente associado");
	...
	public static void adicionarCliente(Cliente c) {
        if (clientesCPF.containsKey(c.getCpf())) 
            throw new IllegalArgumentException("Cliente com CPF " + c.getCpf() + " ja existe");
	...
	public static void removerCliente(Cliente c) {
        if (!clientesCPF.containsKey(c.getCpf())) 
            throw new IllegalArgumentException("Cliente com CPF " + c.getCpf() + " nao existe");
        if (c.getConta() != null && contasPIKS.containsKey(c.getConta().getChavePiks()))
            throw new IllegalArgumentException("Cliente com CPF " + c.getCpf() + " possui uma conta associada");
    ...
    //Metodo implementado pelo professor
